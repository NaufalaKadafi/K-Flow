...
function prettyLabel(item) {
  ...
  if (item?.kind === "auto" && (raw === "tab_out" || raw === "tabout"))
    return "Nyasar tab";
...
export default function CatatDistraksi() {
  const { logDistraction, ledger, stage } = useTimer();
  ...
  const visibleLedger = (ledger ?? []).filter((it) => {
    ...
    if (it?.kind === "auto" && (raw === "tab_in" || raw === "tabin"))
      return false;
    return true;
  });
        ...
        {QUICK.map((q) => {
          ...
          return (
            <button
              ...
              title={
                disabled
                  ? "Hanya bisa digunakan saat fokus"
                  : `Catat distraksi: ${q}`
              }
            >
              {q === "Lainnya" ? (
